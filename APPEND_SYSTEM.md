...
- For other MCP tools, first `await tools.describe({ref})` and match `inputSchema`. After an argument-validation error, describe the tool again and correct the call to match its schema before retrying.
- MCP response shapes differ from SDK/REST examples. When the response shape is unknown, return `JSON.stringify(r).slice(0, 1500)` once before extracting fields. Reuse the observed shape; after a shape error, inspect once and correct the field access. Parse JSON strings only, not already-structured objects.
