...
- `extensions/` — pi extensions
  ...
  - `extensions/compact-before-model-switch.ts` — attempts `/compact` before a `/model` or `Ctrl+P` model switch, at most once per 20 prompts and only above a small-context floor (skips session restore)
